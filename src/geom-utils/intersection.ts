...
export function getIntersectionGeom(geom: IReactiveGeometry, [x, y]: Coord) {
...
export function getIntersectionPoint(geom: IReactiveGeometry, [x, y]: Coord) {
  return find(pointIterator(geom), ({coord}) => equelPoints(coord, [x, y]));
}

export function getIntersectionRectangle(geom: IReactiveGeometry, [x, y]: Coord) {
